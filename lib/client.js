...
class Client extends EventEmitter {
  ...
  _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length) {
    ...
    if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
       ...
       * });
       */
      this.emit('iAm', {address: address, sourceDeets: result.sourceDeets, deviceId: result.deviceId, maxApdu: result.maxApdu, segmentation: result.segmentation, vendorId: result.vendorId});
  ...
  whoIs(options) {
    ...
    this._transport.send(buffer.buffer, buffer.offset, JSON.parse(settings.address).adr);
  }
  ...
  readProperty(address, objectId, propertyId, options, next) {
	...
	const addressIp = addressObject.deviceIp;
	const deviceSource = JSON.stringify(addressObject.deviceSource);
		
    baNpdu.encode(buffer, baEnum.NpduControlPriority.NORMAL_MESSAGE | baEnum.NpduControlBits.EXPECTING_REPLY, deviceSource, null, DEFAULT_HOP_COUNT, baEnum.NetworkLayerMessageType.WHO_IS_ROUTER_TO_NETWORK, 0);
    ...
    baBvlc.encode(buffer.buffer, baEnum.BvlcResultPurpose.ORIGINAL_UNICAST_NPDU, buffer.offset);
    this._transport.send(buffer.buffer, buffer.offset, addressIp);
